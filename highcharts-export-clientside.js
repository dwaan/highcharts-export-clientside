(function(H) {
  ...
  var buildSupportStatus = function() {
    var hasDownloadOrBlob = browserSupportDownload || browserSupportBlob || browserSupportSafari;
    ...
    supportStatus[MIME_TYPES.JPEG] = /* useless, see last param: hasDownloadOrBlob && */
        svgSupport && canvgSupport && (browserSupportDownload || browserSupportSafari);
  ...
  H.Chart.prototype.exportChartLocal = function(options, chartOptions) {
    ...
    // Image processing
      ...
      if (type == MIME_TYPES.SVG) {
        ...
        data.datauri = (browserSupportDownload || browserSupportSafari) && canvas.toDataURL && canvas.toDataURL(type);
        data.blob = (type == MIME_TYPES.PNG) && !browserSupportDownload && !browserSupportSafari && canvas.msToBlob && canvas.msToBlob();
      }
        ...
        data.datauri = (browserSupportDownload || browserSupportSafari) && doc.output('datauristring');
        data.blob = !browserSupportDownload && !browserSupportSafari && doc.output('blob');
      }
    }
